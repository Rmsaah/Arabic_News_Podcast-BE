...
/**
 * REST controller for managing episode playback progress and listening statistics. Handles progress
 * tracking, completion status, and analytics for both users and episodes.
 *
 * <p>This controller consolidates all progress-related functionality including:
 *
 * <ul>
 *   <li>Progress tracking (where user stopped/paused)
 *   <li>Listening time tracking
 *   <li>Episode completion marking
 *   <li>User statistics and analytics
 *   <li>In-progress episodes (Continue Listening feature)
 * </ul>
 */
@RestController
...
public class EpisodeProgressController {
  ...
  /**
   * Update episode progress (position where user stopped/paused). Use this endpoint to save the
   * user's current position in an episode. User is determined from authentication token.
   *
  ...
  public EpisodeProgressDto updateProgress(
      @RequestBody @Valid EpisodeProgressUpdateDto updateDto, Authentication authentication) {

    return episodeProgressService.updateProgress(updateDto, authentication.getName());
  }

  /**
   * Get progress for a specific episode and user. Returns the saved playback position and
   * completion status. User is determined from authentication token.
   *
  ...
  /**
   * Get all in-progress episodes for a user (Continue Listening feature). Returns episodes that the
   * user has started but not completed. User is determined from authentication token.
   *
   * @param authentication Current authenticated user
  ...
  /**
   * Get user's listening statistics. Returns total listening time, episodes completed, etc. User is
  ...
  /**
   * Track listening time for a user. Call this periodically while the episode is playing, or when
   * the user pauses/stops. This updates the total listening time for the user's profile. User is
   * determined from authentication token.
   *
  ...
  /**
   * Update episode progress with exact position (for resume functionality). Use this to save where
   * the user is in the episode for later resumption. User is determined from authentication token.
   *
   * @param episodeId Episode ID
  ...
  /**
   * Mark an episode as completed. Call this when the user reaches the end or explicitly marks it as
   * finished. User is determined from authentication token.
   *
   * @param episodeId Episode ID
