...
/**
 * REST controller for managing users and user profiles. Handles user information retrieval and
 * updates.
 */
@RestController
...
public class UserController {
  ...
  /**
   * Get user information by ID. Users can only view their own profile unless they have ADMIN role.
   *
   * @param id User ID
  ...
  /**
   * Update user's name (first name and/or last name). Users can only update their own profile. At
   * least one field must be provided in the request.
   *
   * @param id User ID
  ...
  public UserDto updateUserName(
    ...
    return userService.updateUserName(
        id, updateDto.firstName(), updateDto.lastName(), authentication.getName());
  }

  /**
   * Get user's complete profile including listening statistics and ratings. Users can only view
   * their own profile unless they have ADMIN role.
   *
   * @param id User ID
   * @param authentication Current authenticated user
